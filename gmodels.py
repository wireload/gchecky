...
class price_t(gxml.Node):
    value    = gxml.Double('', empty=True, default=0)
...
class item_t(gxml.Node):
    ...
    merchant_item_id           = gxml.String('merchant-item', required=False)
...
class address_t(gxml.Node):
    ...
    phone        = gxml.Phone('phone', required=False)
    postal_code  = gxml.Zip('postal-code')
    region       = gxml.String('region') # Region
